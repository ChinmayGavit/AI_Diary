...
const A4midPage = ({ title }) => {

  const handleClick = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8000/api/canva/mp', null, {
      ...
      if (response.status === 201) {
        ...
        setTimeout(() => {
          window.location.href = '/ai/customizeendpage';
        }, 1000);
      } else {
  ...
  return (
    ...
    <A4Container>
      ...
      <ButtonContainer>
        <CustomizeButton to="/ai/customizeendpage" onClick={handleClick}>next</CustomizeButton>
      </ButtonContainer>
    </A4Container>
    </>
  );
