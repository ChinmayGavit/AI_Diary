...
const A4endPage = ({ title }) => {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8000/api/canva/bp', null, {
        withCredentials: true
      });
      // const response = await axios.post('http://localhost:8000/api/canva/fp', {withCredentials:true});
      if (response.status === 201) {
        alert(`Back page saved successfully`);
        setTimeout(() => {
          window.location.href = '/ai/aigenerator';
        }, 1000);
      } else {
  ...
  return (
    ...
    <A4Container>
      ...
      <ButtonContainer>
        <CustomizeButton to="/ai/aigenerator" onClick={handleClick}>next</CustomizeButton>
      </ButtonContainer>
    </A4Container>
    </>
  );
